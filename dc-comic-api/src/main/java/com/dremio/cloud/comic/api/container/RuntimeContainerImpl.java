...
import com.dremio.cloud.comic.api.container.bean.NullBean;
import com.dremio.cloud.comic.api.container.instance.OptionalInstance;
import com.dremio.cloud.comic.api.exception.AmbiguousBeanException;
import com.dremio.cloud.comic.api.exception.MissingContextException;
import com.dremio.cloud.comic.api.exception.NoMatchingBeanException;
...
public class RuntimeContainerImpl implements RuntimeContainer {
    ...
    private final Map<Type, List<ComicBean<?>>> listMatchings = new ConcurrentHashMap<>();
    ...
    public <T> Instance<T> lookup(final Type inType) {
        ...
        if (existing.size() != 1) {
            if (existing.size() > 1) { // enable to override beans using priority (max wins)
                final int max = existing.stream().mapToInt(ComicBean::priority).max().orElse(Integer.MIN_VALUE);
                if (existing.get(1).priority() < max) { // use first bean
                    return doGetInstance(optional, (ComicBean<T>) existing.get(0));
                }
            }

            throw new AmbiguousBeanException("Matching beans for '" + type.getTypeName() + "': " +
                    existing.stream().map(ComicBean::type).map(Type::getTypeName).collect(joining(",", "", ".")));
        }
    ...
    private <T> Instance<T> doGetInstance(final boolean optional, final ComicBean<T> bean) {
